...
import config, smtplib, os, logging
from email.message import EmailMessage

app = Flask(__name__)
app.config.from_object('config')
app.secret_key = app.config.get('SECRET_KEY') or 'devsecret'
...
# create tables immediately (Flask 3 compatible)
with app.app_context():
    db.create_all()
    if not Service.query.first():
        s1 = Service(name='Hajvagas', duration_minutes=30, price=8000)
        s2 = Service(name='Festes', duration_minutes=90, price=20000)
        s3 = Service(name='Borotvalas', duration_minutes=20, price=5000)
        db.session.add_all([s1,s2,s3])
    if not Stylist.query.first():
        st1 = Stylist(name='Fodrasz A', start_hour=9, end_hour=17)
        st2 = Stylist(name='Fodrasz B', start_hour=10, end_hour=15)
...
def create_gcal_event(booking):
    if not app.config.get('GCAL_ENABLED'):
        ...
        return False
    # placeholder for real GCal integration
    log.info('GCal stub create for booking %s', booking.id)
    return False
...
def book_post():
    ...
    db.session.commit()
    if email:
        send_email(email, 'Foglalas visszaigazolas', f'Kedves {name},\n\nSikeresen lefoglaltad: {start_dt} - {service.name}')
    admin_email = app.config.get('ADMIN_EMAIL')
    if admin_email:
        send_email(admin_email, 'Uj foglalas', f'Uj foglalas: {name} - {service.name} - {start_dt} - Tel: {phone}')
